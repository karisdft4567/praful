...
class Customers extends CI_Controller
{
    ...
    { 
       ...
       // print_r($head['clsubgroups_menu']);die;
    ...
    public function edit()
    {
        ...
        $data['customersubgroup'] = $this->customers->subgroup_info($data['customer']['sgid']);

        $data['customergrouplist'] = $this->customers->group_list();
        $data['customersubgrouplist'] = $this->customers->subgroup_list();
        $head['usernm'] = $this->aauth->get_user()->username;
    ...
    public function addcustomer()
    {
        ...
        $this->customers->add($name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $customergroup, $taxid, $name_s, $phone_s, $email_s, $address_s, $city_s, $region_s, $country_s, $postbox_s, $language, $create_login, $password, $docid, $custom, $discount,$loc);
        ...
        {  
               if ($this->input->post('title')) 
               {
                    ...
                    if (!$this->upload->do_upload('userfile')) 
                    {
                       ...
                       // $data['response'] = 1;
                      //  $data['responsetext'] = 'Document Uploaded Successfully. class="btn btn-indigo btn-md"><i class="icon-folder"></i></a>';
                        ...
                        $taxid = $this->input->post('taxid', true);
                        $customergroup = $this->input->post('customergroup');
                        $customersubgroup = $this->input->post('customersubgroup');
                        $name_s = $this->input->post('name_s', true);
                        $phone_s = $this->input->post('phone_s', true);
                        $email_s = $this->input->post('email_s', true);
                        $address_s = $this->input->post('address_s', true);
                        ...
                        if($lastid=$this->customers->addcustomer($data))
                        {
                            $this->customers->adddocument($title,$filename,$lastid);
                            ///// multiple image//////
                    ...
                    $files = $_FILES;
                          ...
                          {           
                                ...
                                $this->customers->adddocument($_POST['titles'][$i],$files['userfiles']['name'][$i],$lastid);                              
                         }
                            ...
                            /////////////////////
                                    ...
                                    if ($create_login) {
                                        ...
                                        $this->customers->adduser($udata);                                     
                                    }

                            //////////////////////
                             //$data['response'] = 1;
                          //$data['responsetext'] = 'Uploaded Successfully.';
...
{   
    ...
    public function editcustomer()
    {
        ...
        $customersubgroup = $this->input->post('customersubgroup', true);
        $taxid = $this->input->post('taxid', true);
        $name_s = $this->input->post('name_s', true);
        $phone_s = $this->input->post('phone_s', true);
        $email_s = $this->input->post('email_s', true);
        ...
        $discount = $this->input->post('discount', true);
        if ($id) {
            $this->customers->edit($id, $name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $customergroup,$customersubgroup,$taxid, $name_s, $phone_s, $email_s, $address_s, $city_s, $region_s, $country_s, $postbox_s, $docid, $custom, $language, $discount);
        }
    }
    ...
    public function document_load_list()
    {
        ...
        foreach ($list as $document) {
            ...
            $row[] = '<a href="' . base_url('userfiles/documents/' . $document->filename) . '" class="btn btn-success btn-xs"><i class="fa fa-file-text"></i> ' . $this->lang->line('View') . '</a> <a class="btn btn-danger btn-xs delete-object" href="#" data-object-id="' . $document->id . '"> <i class="fa fa-trash"></i> </a> <a href="' . base_url('userfiles/documents/' . $document->filename) . '" class="btn btn-success btn-xs" download><i class="fa fa-arrow-down"></i> ' . $this->lang->line('Download') . '</a>';
    ...
    public function adddocument()
    {
        ...
        if ($this->input->post('title')) {
            ...
            {
                 $data['response'] = 0;
                $data['responsetext'] = 'Upload Invalid Filetype';
            }
