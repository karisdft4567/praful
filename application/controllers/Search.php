...
class Search extends CI_Controller
{
    ...
    public function customer()
    {
        ...
        if ($name) {
            $query = $this->db->query("SELECT id,name,address,city,docid,email FROM geopos_customers WHERE $whr (UPPER(name)  LIKE '%" . strtoupper($name) . "%' OR UPPER(docid)  LIKE '" . strtoupper($name) . "%') LIMIT 6");
                ...
                echo ' 
                    ...
                    <a href="' . base_url('customers/view?id=' . $row['id']) . '" class="list-group-item">  <div class="media">
                        ...
                        <div class="media-body">
                          ...
                          <p class="notification-text font-small-3 text-muted">' . $row['address'] . ',' . $row['city'] . '</p><small><i class="icon-phone"></i> ' . $row['docid'] . '</small>
                        </div>
                      </div></a>
