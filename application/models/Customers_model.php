...
class Customers_model extends CI_Model
{
    ...
    private function _get_datatables_query_sub($id = '')
    {  
        $due = $this->input->post('due');
        if ($due) {
            ...
            if ($id != '') { 
                $this->db->where('geopos_customers.sgid', $id);
            }
            $this->db->group_by('geopos_invoices.csd');
            $this->db->order_by('total', 'desc');
    ...
    function get_datatables_subgroups($id = '')
    { 
        $this->_get_datatables_query_sub($id);
        if ($this->aauth->get_user()->loc) {
           //$this->db->where('loc', $this->aauth->get_user()->loc);
        }
        if ($this->input->post('length') != -1)
    ...
    public function add($name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $customergroup, $taxid, $name_s, $phone_s, $email_s, $address_s, $city_s, $region_s, $country_s, $postbox_s, $language = '', $create_login = true, $password = '', $docid = '', $custom = '', $discount = 0,$loc)
    {
        ...
        if (!$valid['email']) {
            ...
            if ($this->db->insert('geopos_customers', $data)) {
               ...
               // $this->customers->adddocument($title, $filename, $cid);
                // echo $this->adddocument($title, $filename, $cid);
                /////////////
                $p_string = '';
                $temp_password = '';
                if ($create_login) {
...
/////////////////////////    
    ...
    public function edit($id, $name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $customergroup, $customersubgroup, $taxid, $name_s, $phone_s, $email_s, $address_s, $city_s, $region_s, $country_s, $postbox_s, $docid = '', $custom = '', $language = '', $discount = 0)
    {
        $data = array(
            'name' => $name,
            'company' => $company,
    ...
    { //echo "asd";die;
        ...
        $query = $this->db->query("SELECT c.*,p.pc FROM geopos_cust_group AS c LEFT JOIN ( SELECT gid,COUNT(gid) AS pc FROM geopos_customers $whr GROUP BY gid) AS p ON p.gid=c.id where c.loc=".$this->aauth->get_user()->loc);
        return $query->result_array();
    }

    public function subgroup_list()
    { //echo "asd";die;
        //echo $this->aauth->get_user()->loc;die;
        $whr = "";
        if ($this->aauth->get_user()->loc) {
            $whr = "WHERE (geopos_customers.loc=" . $this->aauth->get_user()->loc . " ) ";
            if (BDATA) $whr = "WHERE (geopos_customers.loc=" . $this->aauth->get_user()->loc . " OR geopos_customers.loc=0 ) ";
        ...
        $query = $this->db->query("SELECT c.*,p.pc,cg.title as parent_title,c.parent_id FROM geopos_cust_subgroup AS c LEFT JOIN ( SELECT sgid,COUNT(sgid) AS pc FROM geopos_customers $whr GROUP BY sgid) AS p ON p.sgid=c.id
        
        
        LEFT JOIN  geopos_cust_group AS cg ON  cg.id=c.parent_id
         where c.loc=".$this->aauth->get_user()->loc);
    ...
    public function subgroup_info($id)
    {
